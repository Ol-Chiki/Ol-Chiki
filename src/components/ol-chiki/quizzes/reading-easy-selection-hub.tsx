...
interface ReadingEasyQuizSelectionHubProps {
...
const READING_QUIZ_EASY_SCORES_STORAGE_KEY_PREFIX = 'olChikiReadingEasyScores_';

export default function ReadingEasySelectionHub({ onSelectQuiz, onBack }: ReadingEasyQuizSelectionHubProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const storageKey = `${READING_QUIZ_EASY_SCORES_STORAGE_KEY_PREFIX}${userId}`;
      try {
        const storedScores = JSON.parse(localStorage.getItem(storageKey) || '{}');
        setQuizScores(storedScores);
      } catch (error) {
        console.error("Error loading easy quiz scores from localStorage:", error);
        setQuizScores({});
      }
    }
  }, [user]);
    ...
    <div className="p-4 md:p-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-primary tracking-tight">
          Easy: Match Word to Image - Select Set
        </h2>
