...
import { ArrowRight, Edit3, ArrowLeft } from 'lucide-react'; // Changed icon
...
const WRITING_QUIZ_BASIC_SCORES_STORAGE_KEY_PREFIX = 'olChikiWritingQuizScores_Basic_';
...
export default function WritingBasicSelectionHub({ onSelectQuiz, onBack }: WritingBasicSelectionHubProps) {
  ...
  const getStorageKeyForSet = useCallback((quizSetNumber: number) => {
    ...
    return `${WRITING_QUIZ_BASIC_SCORES_STORAGE_KEY_PREFIX}${userId}_set${quizSetNumber}`;
  }, [user]);
